...
class TubeCharacteristics(Component):
    def __init__(self):
        ...
        self.add_param('unit_cost',
                       val=.1322,
                       units='USD/kg',
                       desc='cost of materials per unit mass')
        ...
        self.add_param('dx',
                       val=500.0,
                       units='m',
                       desc='distance between tubes')

        self.deriv_options['type'] = 'user'

        #Define outputs
        self.add_output('m_tube',
                        val=100.0,
                        units='kg',
                        desc='total mass of the tube')
        self.add_output('VonMises',
                        val=0.0,
                        units='Pa',
                        desc='max Von Mises Stress')
        self.add_output('materials_cost',
                        val=0.0,
                        units='USD',
                        desc='cost of tube materials')
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        sig_axial = (((p_tunnel - p_ambient) * r) / (2 * t)) + (
            (q * dx) / (8 * Z))

        unknowns['VonMises'] = (.5 * ((sig_axial**2) + (sig_theta**2) + (
            (sig_axial - sig_theta)**2)))**.5
...
class PylonCharacteristics(Component):
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        unknowns['dx'] = (((pi**3) * E * (r_pylon**4)) /
                          (8 * (h**2) * sf)) / (rho_tube * pi * ((
                              (r + t)**2) - (r**2)) * g)
    ...
    def linearize(self, params, unknowns, resids):
        ...
        J['dx, r_pylon'] = (((pi**3) * E * (r_pylon**3)) /
                            (2 * (h**2) * sf)) / (rho_tube * pi * ((
                                (r + t)**2) - (r**2)) * g)
...
if False:  #__name__ == '__main__':
    ...
    params = (('r', 1.1, {'units': 'm'}), ('t', 0.05, {'units': 'm'}),
              ('dx', 500., {'units': 'm'}), ('Su', 400.0e6, {'units': 'Pa'}),
              ('sf', 1.5), ('p_ambient', 101300.0, {'units': 'Pa'}),
    ...
    root.add(
        'con1',
        ExecComp('c1 = (Su/sf) - VonMises'))  #Impose yield stress constraint
    root.add('con2', ExecComp(
        'c2 = (p_ambient - p_tunnel) - (E/(4*(1-v**2)))*((t/r)**3)'))  #Impose buckling constraint
...
if __name__ == '__main__':
    ...
    g1.add(
        'con1',
        ExecComp('c1 = (Su/sf) - VonMises'))  #Impose yield stress constraint
    g1.add('con2', ExecComp(
        'c2 = (p_ambient - p_tunnel) - (E/(4*(1-v**2)))*((t/r)**3)'))  #Impose buckling constraint
    g1.add('con3', ExecComp(
        'c3 = (Su/sf)-(R/(pi*(r_pylon**2)))'))  #Impose yield stress constraint on pylon
