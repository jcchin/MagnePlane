...
class Vacuum(Component):
    def __init__(self):
        ...
        self.add_param('V', 40.0, desc='volume flow rate of process gas being evacuated per pump', units='m**3/hr')
        ...
        self.add_output('pwr', 1.0, desc='total power consumption (brake kilowatts)')
    ...
    def solve_nonlinear(self, p, u, r):
        rho = p['P']/(287.058*p['T'])
        G = rho*p['V']
        m = G*np.sqrt(p['T']*28.96 / (293.15*p['MW'])) # Air equivalent flow rate, kg/h
        u['n'] = np.ceil(p['leakage']/p['V'])
        u['SF'] = m/(p['P']*0.00750062); # Pump size factor kg/ h/torr
        u['BkW'] = 13.5*(u['SF'])**1.088 # Rough estimate for a rotary piston vacuum pump. SF should be between 0.01 - 4.
        u['pwr'] = u['BkW'] * u['n']
