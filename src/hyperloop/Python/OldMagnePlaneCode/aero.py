...
class Aero(Component):
    ...
    def __init__(self):
        ...
        self.add_param('p_inf',
                       3497.8,
                       desc='Freestream static pressure',
                       units='Pa')
        self.add_param('t_inf',
                       291.111,
                       desc='Freestream static temperature',
                       units='K')
        self.add_param('a_inf',
                       342.241,
                       desc='Freestream reference speed of sound',
                       units='m/s')
        self.add_param('M_inf', 0.8, desc='Freestream Mach no.')
        self.add_param('Re',
                       223789.835,
                       desc='Reynolds no. per unit grid length')
        self.add_param('SPR', 1.136, desc='Fan face static pressure ratio')
        self.add_param('MFR',
                       5.803,
                       desc='Compressor mass flow rate (actual)',
                       units='kg/s')
        self.add_param('TPR',
                       18.865,
                       desc='Nozzle plenum total pressure ratio')
        self.add_param('TTR',
                       2.407,
                       desc='Nozzle plenum total temperature ratio')
        self.add_param(
            'Lstar_Lref',
            1.0,
            desc='Ratio of dimensional reference length to grid length')

        self.add_param('coef_drag', 1.0, desc='capsule drag coefficient')
        self.add_param('area_frontal',
                       1.5,
                       desc='frontal area of capsule',
                       units='m**2')
        self.add_param('velocity_capsule',
                       600.0,
                       desc='capsule velocity',
                       units='m/s')
        self.add_param('gross_thrust',
                       0.0,
                       desc='nozzle gross thrust',
                       units='N')

        self.add_output('net_force',
                        0.0,
                        desc='net force with drag considerations',
                        units='N')
