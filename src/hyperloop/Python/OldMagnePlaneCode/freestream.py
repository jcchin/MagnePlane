...
class Freestream(Component):
    ...
    def solve_nonlinear(self, p, u, r):
        ...
        print("-------------------------------------")
        print("MN = %.6f " % (p['M']))
        print("Ps = %.3f Pa     :  = %.6f psi" %
              (p['Ps'], p['Ps'] * 0.000145038))
        print("Ts = %.4f K      :  = %.4f R" % (p['Ts'], p['Ts'] * 1.8))
        print("Ds = %.6f kg/m^3 :  = %.6f lb/ft^3" %
              (u['Ds'], u['Ds'] * 0.0624279606))
        print(" ")
        print("-------------------------------------")
        print(" --- Freestream Total Conditions ----")
        print("-------------------------------------")
        print("Pt = %.3f Pa     :  = %.3f psi" %
              (u['Pt'], u['Pt'] * 0.000145038))
        print("Tt = %.4f K      :  = %.4f R" % (u['Tt'], u['Tt'] * 1.8))
        print("Dt = %.6f kg/m^3 :  = %.6f lb/ft^3" %
              (u['Dt'], u['Dt'] * 0.0624279606))
