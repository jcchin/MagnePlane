...
def f(M, *data):
    gamma, a_ratio = data
    return (((gamma + 1) / 2)**(-(gamma + 1) / (2 * gamma - 2)) * (
        (1 + (gamma - 1) / 2 * M**2)**(
            (gamma + 1) / (2 * gamma - 2))) / M - a_ratio)
...
def g(M2, *data):
    ...
    ps2 = pt2 * ((1 + (gamma_inf - 1) / 2 * M2**2)**(-gamma_inf /
                                                     (gamma_inf - 1)))  # Pa
...
class Fun3D(Component):
    ...
    def execute(self):
        ...
        ptL = self.p_inf / ((1 + (gamma_inf - 1) / 2 * self.M_inf**2)**
                            (-gamma_inf / (gamma_inf - 1)))  # Pa
        ttL = self.t_inf / (
            (1 + (gamma_inf - 1) / 2 * self.M_inf**2))** -1  # K
        ...
        M2 = fsolve(g,
                    self.M2,
                    args=(self.pt2_ptL + dP, self.tt2_ttL, ptL, ttL, gamma_inf,
                          self.R, self.A2, self.mdot))[0]

        pt2 = ((self.pt2_ptL + dP) * ptL)  # Pa
        tt2 = (self.tt2_ttL * ttL)  # K

        ps2 = pt2 * ((1 + (gamma_inf - 1) / 2 * M2**2)**
                     (-gamma_inf / (gamma_inf - 1)))  # Pa
        ts2 = tt2 * ((1 + (gamma_inf - 1) / 2 * M2**2))** -1  # K
        ...
        print("Plenum Diameter = %f m = %f ft" %
              (diameters[0] * 0.3048, diameters[0]))
        print("Throat Diameter = %f m = %f ft" %
              (np.min(diameters) * 0.3048, np.min(diameters)))
        print("Exit Diameter   = %f m = %f ft" %
              (diameters[-1] * 0.3048, diameters[-1]))
        ...
        pe_0 = self.mdot * np.sqrt(self.T4_0) / (ae_m * np.sqrt(
            gamma_e / self.R) * Me * (1 + (gamma_e - 1) / 2 * Me**2)**
                                                 (-(gamma_e + 1) /
                                                  (2 * gamma_e - 2)))
        pp_0 = self.mdot * np.sqrt(self.T4_0) / (ap_m * np.sqrt(
            gamma_p / self.R) * Mp * (1 + (gamma_p - 1) / 2 * Mp**2)**
                                                 (-(gamma_p + 1) /
                                                  (2 * gamma_p - 2)))

        # --- Static pressure at plenum and exit
        pe = pe_0 * (
            (1 + (gamma_e - 1) / 2 * Me**2)**(-gamma_e / (gamma_e - 1)))
        pp = pp_0 * (
            (1 + (gamma_p - 1) / 2 * Mp**2)**(-gamma_p / (gamma_p - 1)))
        ...
        mustar = 0.00001716 * (self.t_inf / 273.15)**1.5 * (273.15 + 110.4) / (
            self.t_inf + 110.4)  # --- Sutherlands Law
