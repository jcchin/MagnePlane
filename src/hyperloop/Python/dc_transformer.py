...
class DCTransformer(Component):
    def __init__(self):
        ...
        self.add_param('OutputFreq',
                       60,
                       desc='DC output frequency',
                       units='Hz')
        self.add_param('DesignPower', 12000000, units='design output power')
        self.add_param('InputCurrent',
                       2,
                       desc='DC input current',
                       units='Amps')

    def solve_nonlinear(self, params, unknowns, resids):
        input_power = params['InputVoltage'] * params['InputCurrent']
        output_power = params['OutputVoltage'] * params['OutputCurrent']
