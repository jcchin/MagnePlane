...
class TunnelCost(Component):
    ...
    """Computes the unknown values for the given parameter values and put them into the unknown vector

    Computes the 'cost' unknown using the parameters defined in `params` defined at initialization as inputs and
    inserts it into the unknowns vector `unknowns`.


    Args
    ----------
    params : dict
        a mapping of an OpenMDAO param's name (i.e. Defaults.var_name.name) to it's parameter entry in the OpenMDAO model
    params : dict
        a mapping of an OpenMDAO unknown'sh name to it's parameter entry in the OpenMDAO model

    Notes
    -----
    For computing the cost of the tunnel, a relationship derived from [1]_ is used.

    More research should be done on the validity and universalizability of this approach on the cost
    of tunnels in various regions and whether or not it accurately reflecfts today's prices.
